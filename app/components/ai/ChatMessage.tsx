  ...
  return (
    <div className="rounded-xl border border-(--chroma-amber)/25 bg-(--chroma-amber)/4 overflow-hidden">
      ...
      <div className="px-3 py-2 border-b border-white/5 flex items-center gap-2">
        ...
        <span className="font-mono text-[11px] uppercase tracking-wider text-(--chroma-amber)">
      ...
      <div className="p-3 text-[11px] font-mono leading-relaxed space-y-1">
        {/* Old content */}
        {edit.oldContent ? (
          <div className="line-through text-state-critical/60 bg-state-critical/7 px-2.5 py-1.5 rounded-md whitespace-pre-wrap">
            {edit.oldContent}
          </div>
        ) : (
      ...
      {isAccepted && (
        <div className="px-3 pb-2.5 flex items-center justify-center gap-2">
          ...
          {onRevert && (
            <button
              onClick={onRevert}
              className="text-[10px] font-mono text-foreground-muted/30 hover:text-foreground-muted/70 underline underline-offset-2 transition-colors"
            >
              Undo
            </button>
          )}
      ...
      {isRejected && (
        <div className="px-3 pb-2.5 text-foreground-muted/40 text-[10px] text-center font-mono uppercase tracking-wider line-through">
          Changes rejected
        </div>
      )}
    </div>
...
const PRIORITY_BADGE: Record<string, { bg: string; text: string }> = {
  high: { bg: "bg-state-critical/15", text: "text-state-critical/90" },
  medium: { bg: "bg-chroma-amber/15", text: "text-chroma-amber" },
  low: { bg: "bg-chroma-cyan/15", text: "text-chroma-cyan/90" },
};
  ...
  return (
    <div className="rounded-xl border border-(--chroma-indigo)/25 bg-(--chroma-indigo)/4 overflow-hidden">
      ...
      <div className="px-3 py-2 border-b border-white/5 flex items-center gap-2">
        ...
        <span className="font-mono text-[11px] uppercase tracking-wider text-(--chroma-indigo) flex-1">
          {segment.name}
        </span>
        <span
      ...
      <div className="px-3 py-2.5 space-y-1.5">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] text-foreground-muted/50">
          {segment.demographics && (
            <div>
              <span className="font-mono text-foreground-muted/30 uppercase tracking-wider">Demo:</span>{" "}
              {segment.demographics}
            </div>
          )}
          {segment.psychographics && (
            <div>
              <span className="font-mono text-foreground-muted/30 uppercase tracking-wider">Psycho:</span>{" "}
              {segment.psychographics}
            </div>
          )}
          {segment.behavioral && (
            <div>
              <span className="font-mono text-foreground-muted/30 uppercase tracking-wider">Behavior:</span>{" "}
              {segment.behavioral}
            </div>
          )}
          {segment.geographic && (
            <div>
              <span className="font-mono text-foreground-muted/30 uppercase tracking-wider">Geo:</span>{" "}
              {segment.geographic}
            </div>
          )}
        </div>
        ...
        {segment.estimatedSize && (
          <div className="text-[10px] text-foreground-muted/60">
            <span className="font-mono text-foreground-muted/30 uppercase tracking-wider">Est. size:</span>{" "}
            {segment.estimatedSize}
          </div>
        )}
      </div>
  ...
  return (
    <div className="rounded-lg border border-white/8 bg-white/3 overflow-hidden">
      <div className="px-3 py-2 flex items-start gap-2">
        ...
        {isRejected && (
          <span className="text-foreground-muted/30 text-[9px] font-mono uppercase tracking-wider line-through shrink-0">
            Skipped
          </span>
        )}
      </div>
